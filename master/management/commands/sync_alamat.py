    ...
    def handle(self, *args, **options):
        ...
        with open('data/village.json', encoding='utf-8', errors='ignore') as read_village:
            ...
            for row in spamreader:
                ...
                name = row.get('name')
                village_model = Village.objects.filter(id=code, code=code, district_code_id=district_code, name=name).first()
                if village_model:
                    self.stdout.write(
                        self.style.SUCCESS('Village exists')
                    )
                else:
                    province_model = Village.objects.create(id=code, code=code, district_code_id=district_code, name=name)
                    self.stdout.write(
