...
class Jual(FrontPage):
    ...
    def post(self, request):
        ...
        produk = Produk.objects.filter(store__id=store.id, nama=refinput.get("nama", "")).first()
        if produk:
            messages.error(request, "Produk sudah ada")
            return redirect(reverse("jual"))

        verification_input = ["nama", "harga", "deskripsi", "stok", "berat", "lebar", "lintas_negara", "negara", "tipe"]
        for verif in verification_input:
            if refinput.get(verif) is None or refinput.get(verif) == "":
        ...
        try:
            ...
            for reffiles in reffile.getlist("gambar"):
                GambarProduk.objects.create(produk=produk, nama=refinput.get("nama", "-"), gambar=reffiles)
