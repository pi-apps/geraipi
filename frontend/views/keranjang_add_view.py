...
class KeranjangAdd(FrontPage):
    def post(self, request, id=0):
        keranjang_item = CartItem.objects.filter(
            produk__pk=id, cart__user__pk=request.user.id
        )
        ...
        if keranjang_item:
            ...
            keranjang = Cart.objects.create(
                kode=str(time_stamp), user=request.user, status=1
            )
